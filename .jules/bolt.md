...
## 2026-05-18 - Optimize high-frequency SSE event serialization
**Learning:** During chat generation, streaming endpoints can yield hundreds of tokens per second. The `json.dumps()` method from the standard library is relatively slow, and combined with string formatting, it creates unnecessary overhead for each chunk sent to the client.
**Action:** Use `orjson.dumps()` (if available) or `TypeAdapter(Any).dump_json()` along with direct byte concatenations in functions like `_sse_event` to significantly reduce CPU overhead and latency during high-frequency token streaming.
