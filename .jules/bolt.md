...
## 2026-05-10 - Consolidate Sequential Database Inserts
**Learning:** Sequential, unrelated `INSERT` operations into the same table (such as inserting both a user message and an assistant message for a single chat turn) cause unnecessary database roundtrips and execution overhead. Using `execute_many` isn't just for loops; it can be used to batch multiple explicit insert values.
**Action:** When inserting multiple rows into the same table within the same request lifecycle, group them into a single `execute_many` call to reduce database overhead. Ensure nullable columns are properly handled (e.g. passing `None` to map to SQL `NULL`) to maintain parity with the original single-insert column definitions.
