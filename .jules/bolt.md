...
## 2024-05-10 - Eliminate N+1 vectorstore queries
**Learning:** While `asyncio.gather` is a standard way to achieve concurrency, mapping it over database queries in a loop (like searching chunks per-document) causes an N+1 performance bottleneck. Using a single query with an `IN (...)` clause avoids multiple network roundtrips to the DB and allows indexing engines (like pgvector) to evaluate nearest neighbors globally in a single pass.
**Action:** When querying the database for multiple IDs, avoid concurrent loops with `asyncio.gather`. Instead, construct a batched database query using an `IN (...)` clause to minimize connection overhead and optimize query execution.
...
## 2024-08-16 - Avoid full table scans for Analytics Overview
**Learning:** The `/analytics/overview` endpoint previously retrieved entire database tables (users, documents, messages, auth_sessions) into application memory to manually count active entities within the last 7 or 24 hours. This results in heavy I/O network bottlenecks and Python memory exhaustion.
**Action:** When computing summary counts for analytics that require time-based filtering, always push the filtering logic and aggregate logic down to the database using `COUNT(*)` (or `COUNT(DISTINCT)`) combined with `WHERE created_at >= ?` directly in a single `fetch_one` or `fetch_all` call. Do not load large tables into application memory for counting. Furthermore, avoid format differences (like ISO8601 formatting for Postgres vs SQLite) by passing `datetime` objects parameterized instead of manually casting dates into strings.
