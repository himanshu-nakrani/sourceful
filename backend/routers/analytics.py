...
async def analytics_overview(_: RequestContext = Depends(require_admin_context)):
    ...
    # ⚡ BOLT OPTIMIZATION: Batched analytics query
    # Instead of fetching all rows into memory and counting in Python loops,
    # we offload the aggregations and date filtering entirely to the database.
    ...
    active_users_7d = int(counts.get("active_users_7d", 0) or 0)
    signups_7d = int(counts.get("signups_7d", 0) or 0)
    uploads_7d = int(counts.get("uploads_7d", 0) or 0)
    questions_24h = int(counts.get("questions_24h", 0) or 0)
    sessions_24h = int(counts.get("sessions_24h", 0) or 0)
