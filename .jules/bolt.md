...
## 2024-05-14 - Fast SSE Serialization using Pydantic TypeAdapter
**Learning:** For high-frequency Server-Sent Events (SSE) streaming (like LLM token streaming), standard library `json.dumps()` with intermediate `str` interpolation (`f"event: {event}\ndata: {json.dumps(data)}\n\n".encode("utf-8")`) incurs unnecessary CPU overhead and allocation for string encoding. Using `orjson.dumps()` or pre-compiled Pydantic `TypeAdapter.dump_json()` combined with direct byte concatenation drastically reduces this overhead (from ~0.67s to ~0.10s per 100k events).
**Action:** When implementing new streaming endpoints or refactoring existing ones, always use `orjson` or `TypeAdapter(Any).dump_json()` to serialize payloads directly to `bytes` and construct the SSE payload via byte concatenation (`b"event: " + event + b"\n..."`).
